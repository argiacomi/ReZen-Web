...
export function getOverlappingBreakpoints(props) {
  ...
  const overlappingBreakpoints = breakpointKeys.reduce((acc, key) => {
    ...
    for (let prop in normalizedEntries) {
      if (normalizedEntries[prop][key] !== undefined && normalizedEntries[prop][key] !== lastItem[prop]) {
...
export function traverseBreakpoints(breakpointBase, layoutConfig, iterator) {
  const breakpointKeys = typeof breakpointBase.keys === 'function' ? breakpointBase.keys() : breakpointBase.keys;
  ...
  const breakpointsObject = {
    ...
    between: (start, end) => {
      const breakpointKeys =
        typeof breakpointsObject.keys === 'function' ? breakpointsObject.keys() : breakpointsObject.keys;
      const endIndex = breakpointKeys.indexOf(end);
      return breakpointsObject.up(start) + ' and ' + `${endIndex !== -1 && breakpointsObject.down(end)}`;
    },
    only: (key) => {
      const breakpointKeys =
        typeof breakpointsObject.keys === 'function' ? breakpointsObject.keys() : breakpointsObject.keys;
