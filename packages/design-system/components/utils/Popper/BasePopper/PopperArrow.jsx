...
export const PopperArrowRoot = styled('div')(({ theme, ownerState }) => {
  if (!ownerState.disabled) {
    ...
    const innerRadius = Number.parseFloat(theme.rounded.arrowInnerRadius) * ownerState.width;
    const outerRadius = Number.parseFloat(theme.rounded.arrowOuterRadius) * ownerState.width;
    ...
    return {
      ...
      '&::before': {
        ...
        [theme.getColorSchemeSelector()]: {
          backgroundImage: `linear-gradient(${theme.colors.alpha.add(
            theme.colors.white,
            theme.colors.alpha.overlay([ownerState.elevation])
          )},${theme.colors.alpha.add(theme.colors.white, theme.colors.alpha.overlay(ownerState.elevation))})`
        },
        content: '""',
        zIndex: '1 '
      },
