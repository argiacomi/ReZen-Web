...
const Popover = React.forwardRef((props, ref) => {
    ...
    ({ rects }) => {
      ...
      return {
        mainAxis:
          ...
          (arrow && arrowWidth / 2),
        crossAxis:
          -rects.reference.width * referenceOffset.horizontal - rects.floating.width * floatingOffset.horizontal
      };
    },
  ...
  const getAnchorEl = React.useCallback(() => {
    if (anchorReference === 'anchorPosition') {
      if (process.env.NODE_ENV !== 'production' && (!anchorPosition || typeof anchorPosition !== 'object')) {
        console.error(
    ...
    const anchorElement =
      resolvedAnchorEl && resolvedAnchorEl.nodeType === 1 ? resolvedAnchorEl : ownerDocument(popperRef.current).body;

    if (process.env.NODE_ENV !== 'production') {
      const box = anchorElement.getBoundingClientRect();
      if (!import.meta.env.TEST && box.top === 0 && box.left === 0 && box.right === 0 && box.bottom === 0) {
        console.warn(
  ...
  return (
    <RootSlot {...rootProps} {...(!(typeof RootSlot === 'string') && { slotProps: rootSlotPropsProp })}>
