...
const getColorShade = (theme, color, shadeCase = '') => {
  if (!theme || !color) {
    return {};
  }

  const backgroundColors = [
    color === 'inherit' ? 'currentColor' : theme.colors.alpha.lighten(theme.colors[color].body, 0.62),
    color === 'inherit' ? 'currentColor' : theme.colors.alpha.darken(theme.colors[color].body, 0.5)
  ];
  ...
  } else if (shadeCase === 'radial') {
    return {
      backgroundImage: `radial-gradient(${backgroundColors[0]} 0%, ${backgroundColors[0]} 16%, transparent 42%)`,
      [theme.getColorSchemeSelector()]: {
        backgroundImage: `radial-gradient(${backgroundColors[1]} 0%, ${backgroundColors[1]} 16%, transparent 42%)`
      }
    };
  } else {
    return;
...
const LinearProgressRoot = styled('span')(({ ownerState, theme }) => ({
  ...
  ...getColorShade(theme, ownerState.color, 'plain'),
  ...
  ...(ownerState.variant === 'buffer' && {
    ...
    [theme.getColorSchemeSelector()]: { backgroundColor: 'transparent' }
  }),
...
const LinearProgressDashed = styled('span')(
  ({ ownerState, theme }) => ({
    ...
    ...getColorShade(theme, ownerState.color, 'radial'),
...
const LinearProgressBar1 = styled('span')(
  ({ ownerState, theme }) => ({
    ...
    backgroundColor: ownerState.color === 'inherit' ? 'currentColor' : theme.colors[ownerState.color].body,
...
const LinearProgress = React.forwardRef((props, ref) => {
  ...
  if (variant === 'determinate' || variant === 'buffer') {
    if (value !== undefined) {
      ...
      console.error(`You need to provide a value prop when using the determinate or buffer variant of LinearProgress.`);
    }
  }
  if (variant === 'buffer') {
    if (valueBuffer !== undefined) {
      ...
      console.error(`You need to provide a valueBuffer prop when using the buffer variant of LinearProgress.`);
    }
  }
  ...
  const classes = {
    ...
    bar1: [
      ...
      (variant === 'indeterminate' || variant === 'query') && linearProgressClasses.bar1Indeterminate,
  ...
  return (
    <LinearProgressRoot {...linearProgresstRootProps}>
      {variant === 'buffer' ? <LinearProgressDashedComponent {...linearProgressDashedProps} /> : null}
      <LinearProgressBar1Component {...linearProgressBar1Props} />
      {variant === 'determinate' ? null : <LinearProgressBar2Component {...linearProgressBar2Props} />}
    </LinearProgressRoot>
  );
});
