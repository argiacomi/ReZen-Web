...
import { useEnhancedEffect, useEventCallback, useForkRef } from '@hooks';
...
const SwipeableDrawer = React.forwardRef((props, ref) => {
  ...
  const startMaybeSwiping = (force = false) => {
    if (!maybeSwiping) {
      ...
      if (!open && paperRef.current) {
        setPosition(
          getMaxTranslate(horizontalSwipe, paperRef.current) + (disableDiscovery ? 15 : -DRAG_STARTED_SIGNAL),
          {
  ...
  const handleBodyTouchMove = useEventCallback((nativeEvent) => {
    ...
    const velocity =
      ((translate - swipeInstance.current.lastTranslate) / (performance.now() - swipeInstance.current.lastTime)) * 1e3;
  ...
  return (
    <React.Fragment>
      ...
      {!disableSwipeToOpen && variant === 'temporary' && (
        <NoSsr>
          <SwipeArea anchor={anchor} ref={swipeAreaRef} width={swipeAreaWidth} {...SwipeAreaProps} />
        </NoSsr>
      )}
    </React.Fragment>
  );
