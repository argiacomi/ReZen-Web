    ...
    if (typeof value === 'number') {
      style = {
        ...
        width: `calc(100% * ${value} / ${getColumns(ownerState.level - 1)}${
          ownerState.level > 0 && ownerState.container ? ` + ${getSpacing(ownerState.level - 1, 'column')}` : ''
        })`
      };
    }
...
const generateGridStyles = (ownerState) => {
  return {
    ...
    ...(ownerState.container && {
      ...
      margin: `calc(${getSpacing(ownerState.level, 'row')} / -2) calc(${getSpacing(ownerState.level, 'column')} / -2)`,
    ...
    ...((!ownerState.container || (ownerState.level > 0 && ownerState.container)) && {
      ...
      ...((ownerState.disableEqualOverflow || ownerState.parentDisableEqualOverflow) && {
        padding: `${getSpacing(ownerState.level - 1, 'row')} 0px 0px ${getSpacing(ownerState.level - 1, 'column')}`
      })
    })
  };
};
...
const generateOffsetStyles = (ownerState, theme) => {
  const styles = {};
  theme.breakpoints.traverse(ownerState.breakpoints, ownerState.gridOffset, (appendStyle, value) => {
    let style = {};
    if (value === 'auto') {
      style = {
        marginLeft: 'auto'
    ...
    if (typeof value === 'number') {
      style = {
        marginLeft: value === 0 ? '0px' : `calc(100% * ${value} / ${getColumns(ownerState.level - 1)})`
      };
    }
    appendStyle(styles, style);
  });
...
const Grid = React.forwardRef((props, ref) => {
  ...
  const columnSpacing = props.columnSpacing ?? props.spacing ?? (levelZero ? columnSpacingProp : undefined);
