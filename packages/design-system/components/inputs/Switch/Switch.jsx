...
const SwitchRoot = styled('span')(({ ownerState }) => ({
  ...
  ...(ownerState.edge && (ownerState.edge === 'start' ? { marginLeft: '-0.5rem' } : { marginRight: '-0.5rem' })),
  ...(ownerState.small && {
    width: '2.125rem',
    height: '1.125rem',
    [`& .${switchClasses.thumb}`]: {
...
const SwitchSwitchBase = styled(SwitchBase)(({ theme, ownerState }) => ({
  ...
  [`&.${switchClasses.checked} + .${switchClasses.track}`]: {
    ...(ownerState.color === 'success' || ownerState.color === 'warning'
      ? { backgroundColor: theme.colors.alpha.add(theme.colors[ownerState.color][500], 0.9) }
      : { backgroundColor: theme.colors[ownerState.color][500] })
  },
  [`& .${switchClasses.input}`]: {
    left: '-125%',
    width: '325%'
...
const SwitchTrack = styled('span')(({ theme }) => ({
  ...
  backgroundColor: theme.colors.alpha.add(theme.colors.black, 0.25),
  [theme.getColorSchemeSelector()]: {
    backgroundColor: theme.colors.alpha.add(theme.colors.white, 0.55)
  }
}));
