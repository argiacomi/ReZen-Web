...
import { FormControlContext, formControlState } from '../Form';
...
export const InputBaseComponent = styled('input')(({ theme, ownerState }) => {
  ...
  return {
    ...
    [`&.${inputBaseClasses.disabled}`]: {
      opacity: 1,
      WebkitTextFillColor: theme.colors.disabled.text
    },
    '&:-webkit-autofill': {
...
const InputBase = React.forwardRef((props, ref) => {
  ...
  const { checkDirty, focused, formControl, getInputProps, getRootProps, handleInputRef, inputRef, value } =
    useInputBase(props);
  ...
  if (multiline && InputComponent === 'input') {
    if (rows) {
      if (process.env.NODE_ENV !== 'production') {
        if (minRows || maxRows) {
          console.warn(`You can not use the 'minRows' or 'maxRows' props when the input 'rows' prop is set.`);
        }
      }
      inputProps = {
        type: undefined,
