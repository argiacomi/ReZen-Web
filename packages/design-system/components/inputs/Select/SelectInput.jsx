...
import { useControlled, useForkRef } from '@hooks';
import { areEqualValues, capitalize, isFilled, isHostComponent, ownerDocument } from '@utils';
import { Menu } from '@components/navigation/Menu';
import { nativeSelectIconStyles, nativeSelectSelectStyles } from './NativeSelect';

export const selectClasses = {
  select: 'Select-Select',
  icon: 'Select-Icon',
  nativeInput: 'Select-NativeInput',
  disabled: 'Disabled',
  multiple: 'Multiple',
  iconOpen: 'iconOpen',
  error: 'Error'
...
function isEmpty(display) {
  return display == null || (isHostComponent(display) && !display.trim());
}
...
const SelectInput = React.forwardRef((props, ref) => {
  ...
  const items = childrenArray.map((child) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (isFragment(child)) {
        console.error(
          ["The Select component doesn't accept a Fragment as a child.", 'Consider providing an array instead.'].join(
            '\n'
          )
        );
      }
        ...
        slotProps={{
          ...
          popper: {
            ...
            style: {
              ...
              ...(slotProps.menu?.slotProps?.paper != null ? slotProps.menu?.slotProps?.paper?.style : null)
            }
          }
        }}
      >
